    ...
    <div class="container">
        ...
        <form method="GET" action="{{ route('cupom.index') }}" class="row mt-2">
            <div class="col-md-3 mt-1 col-6">
                <input placeholder="DE" name="inicio_filtro" id="inicio_filtro" type="text" data-toggle="datepicker"
                    class="form-control date">
            </div>
            <div class="col-md-3 mt-1 col-6">
                <input placeholder="ATÉ" name="fim_filtro" id="fim_filtro" type="text" data-toggle="datepicker"
                    class="form-control date">
            ...
            <div class="col-md-3 mt-1 col-12 text-center">
                ...
                <button class="mt-1 btn btn-primary text-light" type="submit">Filtrar</button>
            </div>
        </form>
                    ...
                    <tbody>
                        @foreach ($cupons as $c)
                            <tr data-href="{{ route('cupom.visualizar', $c->id) }}" class="clickable"
                                ...
                                <td class="text-center">{{ $c->inicio->format('m/Y') }}
                                    {{-- @if (Auth::user()->nivel_acesso == 1) --}}
                                    @php
                                        $data_finalizado = $c->data_finalizado;
                                    @endphp

                                    @if ($data_finalizado == null)
                                <td class="text-center">Não Finalizada</td>
                            @else
                                <td class="text-center">{{ $data_finalizado->format('d/m/Y H:i:s') }}</td>
                        @endif
                        {{-- @endif --}}
                        </td>
                        @if (Auth::user()->nivel_acesso == 1)
                            <td onclick="event.stopPropagation()" class="text-center">
                                <label id="lbl-pago" class="switch">
                                    <input onclick="altera({{ $c->id }})" {{ $c->pago == 1 ? 'checked' : '' }}
                                        id="btn-pago_{{ $c->id }}" name="pago" type="checkbox">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form id="form-novo" action="{{ Route('cupom.store') }}" method="POST">
                        ...
                        <section class="row mt-2">
                            <div class="col-md-12">
                                <strong>Periodo (Mês/Ano)*</strong>
                                <input id="periodo" autocomplete="off" name="periodo" type="text"
                                    class="form-control date" placeholder="MM/YYYY">
                            </div>
                            @if (Auth::user()->nivel_acesso == 2)
                                <div class="col-md-6 col-6 mt-2">
                                    ...
                                    <input id="km_inicial" name="km_inicial" type="text" class="form-control"
                                        value="{{ $c->km_final ?? '' }}">
                                    <label id="label-inicio-menor" class="text-danger" style="display: none;">KM início
                                        tem que ser maior que o mês passado: "{{ $c->km_final ?? '' }}"</label>
                                </div>
                            @endif
                            @if (Auth::user()->nivel_acesso == 1)
                                <div class="col-md-6 col-6 mt-2">
                            ...
                            <div class="col-md-6 col-6 mt-2">
                                ...
                                <label id="label-final-menor" class="text-danger" style="display: none;">KM Final tem
                                    que ser maior que o KM Inicial</label>
                            </div>
                        </section>
                </div>
