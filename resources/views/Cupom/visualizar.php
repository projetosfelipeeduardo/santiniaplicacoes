...
@section('content')
    ...
    @if (Auth::user()->nivel_acesso == 2 or $dif < 24)
        @php
            $ativado = 1;
        @endphp
    @endif
    ...
    <div class="container">
        ...
        <form id="form-update" action="{{ route('cupom.update', $cupom->id) }}" method="POST" class="row">
            ...
            <div class="col-md-3 col-6">
                <label for="Cidade" class="label">De</label>
        ...
        <form id="form-item" class="row mt-2">
            ...
            <div class="col-md-1 text-center">
                @if ($ativado == 1)
                    <a style="cursor: pointer" id="btn-adicionar" onclick="addItem()" data-placement="top"
                        title="Adicionar Item" data-toggle="tooltip" class="btn btn-primary "><i
        ...
        <div class="row mt-2">
            ...
            <div class="col-md-6 text-center">
                ...
                <table id="table-ft" class="table ">
                    ...
                    <tbody class="tb-foto">
                        @foreach ($imagens as $f)
                            <tr class="tr-foto">
                                <td class="text-center"><a data-toggle="modal" class="badge badge-primary text-light"
                                        style="font-size:12pt; cursor: pointer" data-target="#modal-foto"
                                        onclick="viewFoto('{{ $f->img }}')"
                                        style="cursor: pointer">{{ $f->nome }}.png</a></td>
                                @if ($ativado == 1)
                                    <td class="text-center"><a onclick="removeFoto({{ $f->id }})"
                                            style="cursor: pointer"><i class="fa fa-times text-danger"></i></a></td>
                                @endif
                            </tr>
                        @endforeach
                ...
                @if ($ativado == 1)
                    <button data-toggle="modal" data-target="#modal-pic" class="btn btn-principal text-light mt-2">Enviar
                        Imagem
                    </button>
                @endif
            ...
            <div class="col-md-6 text-center">
                ...
                @if ($ativado == 1)
                    <button data-toggle="modal" data-target="#modal-doc" id="btn-doc"
                        class="btn btn-principal text-light mt-2">Enviar PDF
                    </button>
                @endif
                        ...
                        success: function(data) {
                            if (data['cod'] == 1) {
                                ...
                                $('#tbody-itens').append('<tr><td class="text-center">' + data['contador'] +
                                    ...
                                    '</td><td class="text-center"><a onclick="removeItem(' + data['id'] +
