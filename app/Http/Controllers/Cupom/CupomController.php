...
class CupomController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
          ...
          // Supondo que $data['periodo'] está no formato MM/YYYY
            $periodo = Carbon::createFromFormat('m/Y', $data['periodo']);

           // Cria um novo objeto Carbon para o primeiro dia do mês
            $inicio = Carbon::create($periodo->year, $periodo->month, 1, 0, 0, 0);

            $fim = $periodo->endOfMonth();

            // Formata as datas para os formatos desejados
            $data['inicio'] = $inicio->toDateTimeString(); // Converte para o formato do banco de dados
            $data['fim'] = $fim->toDateTimeString(); // Converte para o formato do banco de dados;
    ...
    public function uploadImage(Request $request)
    {
        ...
        $path = public_path('images/docs/' . $image_name);
        $data['img'] = $image_name;
    ...
    public function uploadDoc($id, Request $request)
    {
        ...
        if ($request->hasFile('arquivo') && $request->file('arquivo')->isValid()) {
            ...
            if ($extension == '') {
                flash('Erro, o arquivo pode estar corrompido.')->warning();
                return redirect()->back();
            }
    ...
    {
        $file_path_docs = public_path('images/docs/' . $name);
        $file_path_cupons = public_path('images/cupons/' . $name);
        $file_path_despesas = public_path('images/despesas/' . $name);

        // Verifica se o arquivo existe no primeiro caminho especificado
        if (file_exists($file_path_docs)) {
            return response()->download($file_path_docs);
        }

        // Verifica se o arquivo existe no segundo caminho especificado
        if (file_exists($file_path_cupons)) {
            return response()->download($file_path_cupons);
        }

        if (file_exists($file_path_despesas)) {
            return response()->download($file_path_despesas);
        }

        // Se não existir em nenhum dos caminhos, define a mensagem flash
    ...
    public function finalizar(Request $request, $id)
    {
        ...
        // Verifica se o usuário tem permissão para finalizar o cupom (outras validações podem ser adicionadas aqui)
        $cupom->finalizado = $request->finalizado;
        $cupom->data_finalizado = Carbon::now();
        $cupom->save();

        return response()->json(['success' => 'Cupom finalizado com sucesso'], 200);
    }
}
